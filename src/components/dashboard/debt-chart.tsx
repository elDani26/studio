...
export function DebtChart({ transactions: initialTransactions }: DebtChartProps) {
  ...
  useEffect(() => {
    const debtByAccount = creditAccounts.reduce((acc, account) => {
      const debt = initialTransactions
        .filter(t => t.isCreditCardExpense && t.account === account.id)
        .reduce((sum, t) => sum + t.amount, 0);

      const payments = initialTransactions
        .filter(t => t.paymentFor === account.id)
        .reduce((sum, t) => sum + t.amount, 0);
    ...
    const data = Object.values(debtByAccount)
      .filter(item => item.value > 0)
      .sort((a, b) => b.value - a.value);

    setChartData(data);
  }, [initialTransactions, creditAccounts]);
